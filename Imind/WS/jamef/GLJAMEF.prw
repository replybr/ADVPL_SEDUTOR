...
User Function GLJAMEF()
	...
	While .T.
		...
		If lCartaCor
			...
			If nEtiqAte <= MV_PAR04
				If nOpcEti != 2
					ApMsgStop("Para impressão de carta de correção com quantidade de volumes menor do que a NF, informe '2-Etiq.Carta Correção' na opção 'Imprime?' ", "Integração Jamef - Carta de correção")
					Loop
				Else
					lCorrMenos := .T.
				EndIf			
	...
	*/		
		...
		MsAguarde({|| Executa(cFilJamef,cNF,cSerie,nEtiqDe,nEtiqAte,lCartaCor,nQtdAmais,nOpcEti,cLocalImp,lCorrMenos)},"Aguarde...","Buscando Informações NF",.T.)
...
Static Function Executa(cFilJamef,cNF,cSerie,nEtiqDe,nEtiqAte,lCartaCor,nQtdAmais,nOpcEti,cLocalImp,lCorrMenos)
	...
	If lCartaCor .and. (nOpcEti != 1)
		...
		ImpEtiCarta(oEtiq,nEtiqDe,nEtiqAte,nQtdAmais,cLocalImp,lCorrMenos)
	EndIf
...
Static function ImpEtiCarta(oEtiq,nEtiqDe,nEtiqAte,nQtdAmais,cLocalImp,lCorrMenos)
