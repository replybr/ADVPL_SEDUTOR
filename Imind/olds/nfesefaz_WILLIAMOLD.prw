...
//-- Incluido para controlar ponto de entrada PE01NFESEFAZ
cTpNF_ES := cTipo
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
		            ...
		            If cChaveA5 <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI .or. SF4->F4_CF $ '5934 ;5905 '	// se for transferencia não aglutina
		            	nA5 := nA5+1
		            	cChaveA5	:= cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI
						...
						cNCM 	:= SB1->B1_POSIPI
					Else
//						aProd[nA5][9]  := aProd[nA5][9] + (cAliasSD2)->D2_QUANT
//						aProd[nA5][10] := aProd[nA5][10] + IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)) 
//						aProd[nA5][12] := aProd[nA5][12] + IIF ((cAliasSD2)->D2_QTSEGUM > 0, (cAliasSD2)->D2_QTSEGUM, (cAliasSD2)->D2_QUANT)//IIF(Empty(SB5->B5_CONVDIP),(cAliasSD2)->D2_QUANT,SB5->B5_CONVDIP*(cAliasSD2)->D2_QUANT)
						//aProd[nA5][16] := aProd[nA5][16] + aProd[nA5][16] //Alteração para aglutinar itens iguais (Controle de Serie) -- William P. Alves

						aProd[nA5][9]  += (cAliasSD2)->D2_QUANT
						aProd[nA5][10] += IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0))
						aProd[nA5][12] += IIF(Empty(SB5->B5_CONVDIP),(cAliasSD2)->D2_QUANT,SB5->B5_CONVDIP*(cAliasSD2)->D2_QUANT)
											

						aProd[nA5][15] += (nDesconto+nDescIcm+nDescRed)
						aProd[nA5][26] += nDescZF
					...
					cNCM := SB1->B1_POSIPI
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							EndIf										
							//If cChaveICM <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ)
						//	cChaveICM := cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ)										
							Do Case
								...
								If cChaveICM <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ) 
									...
									EndIf
								Else
									aICMS[nicm][5] := aICMS[nicm][5] + If(lNfCupZero,0,CD2->CD2_BC)
									aICMS[nicm][7] := aICMS[nicm][7] + If(lNfCupZero,0,CD2->CD2_VLTRIB)								
								EndIf
...
/*
								...
								EndIf
*/						
						//If cChaveSol <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(CD2->CD2_MVA) + str(CD2->CD2_ALIQ)
						//	cChaveSol := cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(CD2->CD2_MVA) + str(CD2->CD2_ALIQ)		
						 	Case AllTrim(CD2->CD2_IMP) == "SOL" .And. (cAliasSD2)->D2_TIPO<>"I"
					 		//If cChaveSol <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(CD2->CD2_MVA) + str(CD2->CD2_ALIQ)
						 	If lCalSol
						 		nsol := Ascan(aICMSST,{|x|(x[12])==cCodProd})
						 	EndIf
								If nsol > 0   
								//aadd(aICMSST,{"","","",0,0,0,0,0,0,0,"",""})
								aICMSST[nsol][5] := aICMSST[nsol][5] + If(lNfCupZero,0,CD2->CD2_BC)
								aICMSST[nsol][7] := aICMSST[nsol][7] + If(lNfCupZero,0,CD2->CD2_VLTRIB)
								Else								//cChaveSol := cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(CD2->CD2_MVA) + str(CD2->CD2_ALIQ)										
								//nsol := nsol + 1
						  ...
						  //	EndIF
							...
							aTail(aICMSST) := {CD2->CD2_ORIGEM,;
								...
								Case AllTrim(CD2->CD2_IMP) == "IPI"
									If cChaveIPI <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ) 
										cChaveIPI := cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ)										
										nipi := nipi + 1
										If !lConsig
										...
										EndIf
									Else
										aIPI[nipi][6] := aIPI[nipi][6] + If(lNfCupZero,0,CD2->CD2_BC)
										aIPI[nipi][10] := aIPI[nipi][10] + If(lNfCupZero,0,CD2->CD2_VLTRIB)								
									EndIf
								Case AllTrim(CD2->CD2_IMP) == "PS2"
									If cChavePIS <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ) 
										cChavePIS := cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ)										
										npis := npis + 1
										If !lNfCupZero
										...
										EndIf
									Else
										aPIS[npis][2] := aPIS[npis][2] + CD2->CD2_BC  
										aPIS[npis][4] := aPIS[npis][4] + CD2->CD2_VLTRIB
										aPIS[npis][5] := aPIS[npis][5] + CD2->CD2_QTRIB								
									EndIf	
									
								Case AllTrim(CD2->CD2_IMP) == "CF2" 
									If cChaveCOF <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ) 
										cChaveCOF := cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ)
										ncof := ncof + 1
										If !lNfCupZero
										...
										EndIf
									Else
										aCOFINS[ncof][2] := aCOFINS[ncof][2] + CD2->CD2_BC  
										aCOFINS[ncof][4] := aCOFINS[ncof][4] + CD2->CD2_VLTRIB
										aCOFINS[ncof][5] := aCOFINS[ncof][5] + CD2->CD2_QTRIB								
									EndIf
								...
								If cChaveDIFAL <> cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ) 
									cChaveDIFAL := cCodProd+cDescProd+cD2Cfop+SB1->B1_POSIPI + CD2->CD2_ORIGEM + CD2->CD2_CST + CD2->CD2_MODBC + str(nMargem) + str(CD2->CD2_ALIQ)										
									ndifal := ndifal + 1
								...
								Else
										...
										aICMUFDest[ndifal][1] := aICMUFDest[ndifal][1] + CD2->CD2_BC
										aICMUFDest[ndifal][7] := aICMUFDest[ndifal][7] + CD2->CD2_VDDES
										aICMUFDest[ndifal][8] := aICMUFDest[ndifal][8] + CD2->CD2_VLTRIB								
								EndIf
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		...
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
					...
					aAgrPis[nX],aAgrCofins[nX],nIcmsDif,aICMUFDest[nX],@nvFCPUFDest,@nvICMSUFDest,@nvICMSUFRemet,cAmbiente,aIPIDevol[nX],aItemVinc[nX])
	Next nX
