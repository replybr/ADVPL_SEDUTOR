...
±±ºPrograma  ³RELORC ºAutor  ³Erike Yuri       º Data ³  03/04/17         º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Relatorio de Pedido de Venda.                               º±±
±±º          ³                                                            º±±
...
//Alex
oFont17  := TFont():New( "Arial",,09,,.F.,,,,,.F. )
oFont17B := TFont():New( "Arial",,09,,.T.,,,,,.F. )
...
ELSE
		oprn:Say(nLin,1600,"CPF:",oFont09B)
		oprn:Say(nLin,1750,TRANSFORM(SA1->A1_CGC,"@R 999.999.999-99"),oFont09)
ENDIF		
	...
	//	oprn:Box(nLin,1680,nLin+40,2300)
		...
		oprn:Say(nLin+12,0510,"Data de Envio / Recebimento de E-mail",oFont09B)
		oprn:FillRect({nLin,0188,nLin+200,0195 },oBrush) //Linha
		oprn:FillRect({nLin,1395,nLin+200,1395 },oBrush) //Linha
		oprn:FillRect({nLin+205,0188,nLin+200,1395 },oBrush) //Linha
		
		nLin := 3150
		oFont09:Underline := .T.
		//oFont09:nHeight := 11
		oprn:Say(nLin-15,210,"Envio: " + DTOC(SC5->C5_XDTENV) + " as " + SC5->C5_XHRENV,oFont09)
	
		//nLin += nEsp                                      
	
		oprn:Say(nLin-15,703,"Recebimento: "+ DTOC(SC5->C5_XDTLEI) + " as " + SC5->C5_XHRLEI,oFont09)
		//oprn:Line(nLin,300,nLin,600)
		//oprn:Line(nLin,700,nLin,1300)
		//oprn:Say(nLin,700,"Assinatura / Carimbo",oFont09)
		
		nLin := 3300
		oFont09:Underline := .F.	
		//oFont09:nHeight := 09
		oprn:Say(nLin,0180,"Acesse a Política da Qualidade, Segurança, Meio Ambiente e Saúde no site http://www.galaxyled.com.br/institucional",oFont09)
